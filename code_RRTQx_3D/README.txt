...
1. Install Julia v1.0.5, VSCode, and the Julia plugin.
...
3. Run sim_RRTQX() in experimentsForRRTQX.jl to perform RRT-QX. Change the start, goal, environment (obstacles) in sim_RRTQX().

4. The Q-learning function is embedded in function moveRobot_Q() in Q-learning.jl. For visualization in simulations, you need to call different Q-learning functions to implement 
different control algorithms. For simplicity, once the Q-learning function is called, the whole trajectory for one BVP is calculated. Then it is gradually shown at every frame. 
But for implementation on real robots, robots will move as the papers describe.

5. Run make_video_Q.m to create a video of motion planning using RRT-QX.
